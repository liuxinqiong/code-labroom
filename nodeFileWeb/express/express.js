...
function resWrite(response, statusCode, mimeKey, data) {
	response.set('Content-Type',mime(mimeKey));
...
app.all('*', function(request, response){
    ...
    var realPath = path.resolve(path.join('www', pathname));//请求的实际路径
                    ...
                    if (isExistsIndex) {
                        ...
                        return getPromise(function (resolve, reject) {
                            ...
                            Promise.all(pmlist).then(function (linkList) {
                                ...
                                links += linkList.join('');
                                links += '</ul>';
                                var dirPage = `<!doctype html><html><head><meta charset="utf-8"/><style>a{color:blue;text-decoration: none;}.dir a{color:orange}</style></head><body>${links}</body></html>`;
